...
export default function Raulopoly() {
  ...
  const loadGame = useCallback(() => {
    let savedGameRaw = null;
    try {
      savedGameRaw = localStorage.getItem('raulopolyGame');
    } catch (_) {
      return;
    }

    if (!savedGameRaw) return;

    try {
      const parsedGameState = JSON.parse(savedGameRaw);
      setNumPlayers(parsedGameState.numPlayers);
      setPlayers(parsedGameState.players);
      setCurrentIdx(parsedGameState.currentIdx);
      setPhase(parsedGameState.phase);
      setDice(parsedGameState.dice);
      setChaosDie(parsedGameState.chaosDie);
      setPropOwners(parsedGameState.propOwners);
      setPropHouses(parsedGameState.propHouses);
      setLog(parsedGameState.log);
      setJackpot(parsedGameState.jackpot);
      setActiveCard(parsedGameState.activeCard);
      setDoubleRentTurns(parsedGameState.doubleRentTurns);
      setPendingBuy(parsedGameState.pendingBuy);
      setPendingRent(parsedGameState.pendingRent);
      setWinner(parsedGameState.winner);
      setPlayerNames(parsedGameState.playerNames);
      setInitialMoney(parsedGameState.settings.initialMoney);
      setRentMultiplier(parsedGameState.settings.rentMultiplier);
      setChaoschance(parsedGameState.settings.chaosChance);
      setScreen('game');
    } catch (e) {
